...
export function autenticarProfessor(req, res, next) {
    ...
    try {
        ...
        if (payload.tipo !== 'professor') {
            return res.status(403).send({ success: false, erro: 'Acesso negado. Apenas professor.' });
        }

        req.professorId = payload.id;
        next();
    } catch (err) {
